...
void Open::init()
{
	...
	glGenVertexArraysOES(1, &vaoObject1);
	glBindVertexArrayOES(vaoObject1);
...
//    glClearDepth(1.0f);
	...
	//prepare the scaling matrix for object 1
	static float sx=0,sy=0,sz=0;
	sx+=0.01f;
	sy+=0.01f;
	sz+=0.01f;
	glm::mat4 transformMatrix = glm::mat4(1.0f);
	transformMatrix[0].x = sin(sx)+2;
    transformMatrix[1].y = cos(sx)+2;
    transformMatrix[2].z = sin(sz)/2+cos(sz)/2+2;
...
//	glDrawElements(GL_TRIANGLES, ARRAY_COUNT(indexData), GL_UNSIGNED_SHORT, 0);
	...
	//prepare the scaling matrix for object 2
	transformMatrix[0].x = cos(sx)+2;
    transformMatrix[1].y = sin(sx)+2;
    transformMatrix[2].z = cos(sz)/2+sin(sz)/2+2;
	glUniformMatrix4fv(modelToCameraMatrixUnif, 1, GL_FALSE, &transformMatrix[0][0]);
	//render object 2
	glDrawElements(GL_TRIANGLES, ARRAY_COUNT(indexData), GL_UNSIGNED_SHORT, 0);
